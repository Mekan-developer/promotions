<div class="w-full flex justify-center items-center shadow-md sm:rounded-sm p-2">
    <div class="relative bg-white dark:bg-gray-900 p-4 h-full w-full overflow-x-auto" wire:loading.remove wire:target="update, cancel">
        <div class="overflow-y-auto overflow-hidden h-full">
            <div class="mb-2">
                <h1 class="block mb-2 uppercase font-medium text-gray-900 dark:text-gray-200">Edit Administrator</h1>
            </div>
                    ...
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label wire:click="toggleRole('admin')" class="cursor-pointer">
                                ...
                                <span class="{{ ($role_is == 'admin') ? 'bg-blue-700 hover:bg-blue-800 text-white': 'hover:bg-gray-700 text-gray-900 dark:text-white border-gray-300  dark:border-gray-600' }} active:scale-[0.95] inline-block px-4 py-2 border rounded-md ">Admin</span>
                            </label>
                            
                            <label wire:click="toggleRole('editor')" class="cursor-pointer">
                                <input type="radio" wire:model="roles" value="editor" class="hidden"/>
                                <span class="{{ ($role_is == 'editor') ? 'bg-blue-700 hover:bg-blue-800 text-white': 'hover:bg-gray-700 text-gray-900 dark:text-white border-gray-300  dark:border-gray-600' }} active:scale-[0.95] inline-block px-4 py-2 border rounded-md ">Editor</span>
                            </label>
                            
                            <label wire:click="toggleRole('viewer')" class="cursor-pointer">
                                <input type="radio" wire:model="roles" value="viewer" class="hidden"/>
                                <span class="{{ ($role_is == 'viewer') ? 'bg-blue-700 hover:bg-blue-800 text-white': 'hover:bg-gray-700 text-gray-900 dark:text-white border-gray-300  dark:border-gray-600' }} active:scale-[0.95] inline-block px-4 py-2 border rounded-md ">Viewer</span>
                            </label>
                        </div>
